...
public:
  ModelExplorer() : _aModelIsSelected(false), _downloadBar(_downloadProgress) {
    // GUI
    ...
    _downloadLabel.setColour(Label::ColourIds::textColourId, DARKER_STRONG);
    ...
    addAndMakeVisible(_description);
    addChildComponent(_downloadBar);
    addChildComponent(_downloadLabel);
  }
  ...
  void _setDownloadDisplay() {
    _downloadButton.setEnabled(false);
    _importButton.setEnabled(false);
    _description.setVisible(false);
    _modelsList.setVisible(false);
    _modelName.setVisible(false);
    _info.setVisible(false);
    _descriptionLabel.setVisible(false);
    _description.setVisible(false);
    _downloadBar.setVisible(true);
    _downloadLabel.setVisible(true);
  }

  void _setExplorerDisplay() {
    _downloadBar.setVisible(false);
    _downloadLabel.setVisible(false);
    _downloadButton.setEnabled(true);
    _importButton.setEnabled(true);
    _description.setVisible(true);
    _modelsList.setVisible(true);
    _modelName.setVisible(true);
    _info.setVisible(true);
    _descriptionLabel.setVisible(true);
    _description.setVisible(true);
  }

  void displayDownload(String modelName = "", bool reset = true) {
    if (reset) 
      _downloadProgress = 0.;
    if (modelName != "") 
    ...
    _downloadLabel.setText(String("Downloading model") + modelName + String("..."), NotificationType::dontSendNotification);
    repaint();
  }
  ...
  void setDownloadProgress(double download) {
    _downloadProgress = download;
    MessageManagerLock mml (Thread::getCurrentThread());
    while (!mml.lockWasGained()) {
      Thread::sleep(10);
    }
    repaint(_downloadBar.getBoundsInParent());
  ...
  void resized() override {
    ...
    // Position of download items
    auto b_area_2 = getLocalBounds();

    int progressWidth = ((b_area_2.getWidth() - 2 * UI_MARGIN_SIZE) / 2);
    int progressHeight = 24;
    int progressX = (UI_MARGIN_SIZE + progressWidth / 2);
    int progressY = (((b_area_2.getHeight() - 2 * UI_MARGIN_SIZE) / 2) - progressHeight / 2);
    _downloadBar.setBounds(Rectangle<int>(progressX, progressY, progressWidth, progressHeight));
    int labelY = (progressY - UI_TEXT_HEIGHT);
    _downloadLabel.setBounds(Rectangle<int>(progressX, labelY, progressWidth, UI_TEXT_HEIGHT));
